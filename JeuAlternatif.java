...
public class JeuAlternatif {
    ...
    public static void choixCombattant(Equipe equipe, Scanner scanner) {
        ...
        for(int i = 4; i > 0; i--) {
            ...
            switch (nbCombattant) {
                case 0 -> equipe.getTeam().add(new Paladin());
                case 1 -> equipe.getTeam().add(new Berserker());
                case 2 -> equipe.getTeam().add(new Mage());
                case 3 -> equipe.getTeam().add(new Archer());
                case 4 -> equipe.getTeam().add(new Voleur());
                case 5 -> equipe.getTeam().add(new Guerrier());
                case 6 -> equipe.getTeam().add(new Pretre());
    ...
    public static void Mobs(Equipe groupe) {
        ...
        for(int i = 0; i < nombre; i++) {
            switch(type) {
                case 0 -> groupe.getTeam().add(new Diablotin());
                case 1 -> groupe.getTeam().add(new Cultist());
                case 2 -> groupe.getTeam().add(new Demon());
            }
        }
    }
            ...
            int choix = scanner.nextInt();
                switch(choix) {
                    ...
                    case 2 -> {
                        ...
                        switch(choixNom) {
                            case "Potion" -> {
                                for( Item i : new HashMap<>(inventaire.getInventaire()).keySet()) {
